...
def check_rate_limit(key_id: int, limit: int | None, window_seconds: int) -> RateLimitResult:
    ...
    # limit=None means unlimited (Enterprise) - skip Redis entirely, always allow
    if limit is None:
    ...
    try:
        ...
        # during Redis downtime becomes a real concern (could add IP-based fallback
        # limiting via in-memory cache as a hybrid approach).
        return RateLimitResult(allowed=True, current=0, retry_after=0)
